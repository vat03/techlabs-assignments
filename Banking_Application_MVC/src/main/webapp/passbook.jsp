				...
				<tbody>
					<c:forEach var="transaction" items="${transactions}">
						<tr>
							...
							<td><c:choose>
									<c:when
										test="${transaction.senderAccountNumber == transaction.receiverAccountNumber}">${transaction.senderBalanceAfter}</c:when>
									<c:when
										test="${transaction.senderAccountNumber == accounts[0].accountNumber}">${transaction.senderBalanceAfter}</c:when>
									<c:otherwise>${transaction.receiverBalanceAfter}</c:otherwise>
								</c:choose></td>
