...
public class TransactionController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("newTransaction".equals(action)) {
            ...
            if (accounts.isEmpty()) {
                request.setAttribute("error", "Transaction failed: You don’t have an account. Please contact an admin to create one.");
                request.getRequestDispatcher("newTransaction.jsp").forward(request, response);
                return;
            }
            ...
            if ("transfer".equals(actionType)) {
                ...
                if (receiverAccount == null) {
                    request.setAttribute("error", "Transaction failed: Receiver account number does not exist.");
                    request.getRequestDispatcher("newTransaction.jsp").forward(request, response);
                    return;
                }
            } else {
